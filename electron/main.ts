...
function startStaticServer(outDir: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    httpServer = createServer((req, res) => {
      ...
      // v11-rebuild: matches mc-panel.tsx for all 4 modes (single, dual,
      ...
      if (urlPath === '/mc') {
        ...
        if (fs.existsSync(mcHtmlPath)) {
          res.writeHead(200, {
            ...
            'X-Saatiril-Version': 'v11-rebuild',
          })
      ...
      // Returns the Saatiril build version (v11-rebuild). MC + operator HTML
      // also expose this via <meta name="saatiril-version"> + console.log.
      if (urlPath === '/version' || urlPath === '/api/version') {
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({ version: 'v11-rebuild', component: 'saatiril-server' }))
        return
      }
