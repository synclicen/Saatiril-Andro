 ...
 *
 * This is a direct port of the Electron operator.html with an
 ...
 * work unchanged.
 *
 * Layout: split-screen (camera LEFT ~65%, sidebar RIGHT ~35%) matching the
 * Electron internal operator panel. The sidebar contains: toolbar (shutter,
 * grid, antrean, fullscreen), target info card, camera selector, collapsible
 * settings panels, and an always-visible scrollable queue list. On mobile
 * (< 768px), the sidebar collapses into a slide-up bottom sheet toggled by
 * a floating button.
 *
 * NOTE: The HTML is split into multiple parts to work around the JVM's
 * 65535-byte constant pool UTF-8 limit. The parts are concatenated at
 * runtime via String concatenation.
 */


private val OPERATOR_HTML_PART_1 = """<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── Operator Screen (split-screen: camera LEFT, sidebar RIGHT) ── */
    ...
    /* LEFT panel: camera zone + capture button */
    .camera-zone {
      flex: 0 0 65%; min-width: 0; display: flex; flex-direction: column;
      background: #000; position: relative;
    }
    .camera-zone .capture-bar { width: 100%; }
    ...
    /* ── Capture bar (full-width button at bottom of left panel) ── */
    .capture-bar {
      display: flex; align-items: center; gap: 6px;
      padding: 6px 10px; background: rgba(42,22,74,0.9); flex-shrink: 0;
    ...
    /* ── OP Search ── */
    .opsearch-wrap { position: relative; padding: 4px 6px; flex-shrink: 0; }
    .opsearch-input {
      width: 100%; height: 30px; border-radius: 6px;
      background: var(--card); border: 1px solid var(--border);
      color: #fff; font-size: 11px; padding: 0 8px 0 28px; outline: none;
    }
    .opsearch-input:focus { border-color: var(--gold); }
    .opsearch-input::placeholder { color: rgba(196,181,253,0.4); }
    .opsearch-icon {
      position: absolute; left: 12px; top: 50%; transform: translateY(-50%);
      font-size: 12px; color: var(--muted); pointer-events: none;
    }
    .opsearch-results { overflow-y: auto; }
    .opsearch-item {
      display: flex; align-items: center; gap: 6px;
      padding: 4px 8px; cursor: pointer; font-size: 10px;
      border-bottom: 1px solid rgba(83,52,133,0.2);
    }
    .opsearch-item:active { background: rgba(212,175,55,0.15); }
    .os-nim { width: 50px; color: var(--muted); font-family: monospace; font-size: 8px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .os-nama { flex: 1; color: #fff; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 10px; }
    .os-nama.active { color: var(--gold); font-weight: 700; }
    .os-status { font-size: 7px; font-weight: 700; width: 28px; text-align: right; }
    .opsearch-empty { font-size: 9px; color: var(--muted); text-align: center; padding: 8px 0; }

    /* ── Queue list (sidebar, always visible, scrollable) ── */
    .queue-header {
      padding: 4px 8px; flex-shrink: 0; display: flex; align-items: center;
      justify-content: space-between; background: var(--card);
      border-bottom: 1px solid var(--border);
    }
    .queue-title { font-size: 10px; font-weight: 700; color: var(--gold); }
    ...
    .sidebar .queue-row {
      display: flex; align-items: center; gap: 2px;
      padding: 4px 6px; font-size: 10px; min-height: 28px;
      border-bottom: 1px solid rgba(83,52,133,0.3);
    }
    .queue-panel .queue-row.active-row,
    .sidebar .queue-row.active-row { background: rgba(212,175,55,0.15); }
    .queue-panel .queue-row.sent-row,
    .sidebar .queue-row.sent-row { background: rgba(212,175,55,0.05); }
    .queue-panel .queue-row.done-row,
    .sidebar .queue-row.done-row { background: rgba(196,181,253,0.03); }
    .queue-panel .q-idx,
    .sidebar .q-idx { width: 18px; color: var(--muted); font-family: monospace; font-size: 9px; }
    .queue-panel .q-nim-col,
    .sidebar .q-nim-col { width: 52px; color: var(--muted); font-family: monospace; font-size: 9px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .queue-panel .q-nama-col,
    .sidebar .q-nama-col { flex: 1; color: #fff; font-size: 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .queue-panel .q-nama-col.active-name,
    .sidebar .q-nama-col.active-name { color: var(--gold); font-weight: 700; }
    .queue-panel .q-nama-col.done-name,
    .sidebar .q-nama-col.done-name { color: var(--muted); text-decoration: line-through; }
    .queue-panel .q-status-col,
    .sidebar .q-status-col { width: 38px; font-size: 8px; font-weight: 700; text-align: right; }
    .queue-panel .queue-empty,
    .sidebar .queue-empty { font-size: 10px; color: var(--muted); text-align: center; padding: 10px 0; }
    ...
    .tool-btn.active { border-color: var(--gold); color: var(--gold); background: rgba(212,175,55,0.15); }
    .tool-btn .tool-badge {
    ...
    /* ── Sidebar Target Info (always visible) ── */
    .sidebar-target {
      display: flex; align-items: center; gap: 8px;
      padding: 8px 10px; background: var(--card);
      border-bottom: 1px solid var(--border); flex-shrink: 0; min-height: 48px;
    }
    .sidebar-target .tb-left { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; }
    .sidebar-target .tb-avatar {
      width: 28px; height: 28px; border-radius: 50%; background: var(--panel);
      border: 2px solid var(--gold); display: flex; align-items: center; justify-content: center;
      font-size: 14px; flex-shrink: 0;
    }
    .sidebar-target .tb-info { flex: 1; min-width: 0; }
    .sidebar-target .tb-name { font-size: 13px; font-weight: 700; color: #fff; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .sidebar-target .tb-nim { font-size: 10px; color: var(--muted); font-family: monospace; }
    .sidebar-target .tb-badge { font-size: 9px; font-weight: 700; padding: 3px 8px; border-radius: 4px; flex-shrink: 0; }
    .sidebar-target .tb-badge.toga { background: rgba(212,175,55,0.2); color: var(--gold); }
    .sidebar-target .tb-badge.ijazah { background: rgba(34,197,94,0.2); color: var(--success); }
    .sidebar-target .tb-badge.sending { background: rgba(196,181,253,0.15); color: var(--muted); }
    .sidebar-target .tb-badge.standby { background: rgba(196,181,253,0.1); color: rgba(196,181,253,0.5); }
    ...
    .sidebar-camera-selector .camera-select {
      flex: 1; padding: 6px 8px; border-radius: 6px; font-size: 11px;
      background: var(--card); border: 1px solid var(--border); color: #fff;
      outline: none; min-width: 0;
    }
    .sidebar-camera-selector .next-cam-btn {
      padding: 6px 10px; border-radius: 6px; font-size: 10px; font-weight: 700;
      background: linear-gradient(135deg, var(--gold), #c49b2e); color: var(--bg);
      border: none; cursor: pointer; white-space: nowrap;
    }
    .sidebar-camera-selector .next-cam-btn:active { opacity: 0.7; }
    .sidebar-camera-selector .rescan-btn {
      padding: 6px 8px; border-radius: 6px; font-size: 10px; font-weight: 700;
      background: var(--card); border: 1px solid var(--border); color: var(--muted);
      cursor: pointer; white-space: nowrap;
    }
    .sidebar-camera-selector .rescan-btn:active { background: var(--border); }

    /* ── Sidebar Collapsible Panels (shutter, gridline settings) ── */
    .sidebar-panel {
      padding: 8px 10px; background: var(--card);
      border-bottom: 1px solid var(--border); flex-shrink: 0;
    }
    .sidebar-panel.hidden { display: none; }
    .sidebar-panel .settings-label { font-size: 10px; font-weight: 700; color: var(--gold); margin-bottom: 6px; }
    .sidebar-panel .settings-row {
      display: flex; align-items: center; justify-content: space-between;
      padding: 4px 0;
    }
    .sidebar-panel .settings-row .settings-label { margin-bottom: 0; }

    /* ── Sidebar Queue (fills remaining space) ── */
    .sidebar-queue {
    ...
    .queue-toggle-mobile {
      ...
      background: linear-gradient(135deg, var(--gold), #c49b2e); color: var(--bg);
      border: 2px solid var(--bg); font-size: 22px; cursor: pointer;
...
<div id="operatorScreen" class="operator-screen">
  <!-- Top Bar (compact: title + mode + target + latency + disconnect) -->
  <div class="top-bar">
  ...
  <!-- ══ Main split-screen: camera LEFT, sidebar RIGHT ══ -->
  <div class="operator-main">
    <!-- LEFT: Camera zone + capture button -->
    <div class="camera-zone">
      <!-- Camera viewport (aspect ratio preserved via object-fit: contain) -->
      <div id="cameraArea" class="camera-area">
        ...
        <!-- Target info overlay (small, on camera — useful in fullscreen) -->
        ...
        <!-- Photo progress dots (on camera, non-photoshoot mode) -->
    ...
    <!-- RIGHT: Sidebar (toolbar + target + camera selector + queue) -->
    <div id="sidebar" class="sidebar">
      <!-- Toolbar: SHUTTER | GRID | ANTREAN | FULLSCREEN -->
      <div class="sidebar-toolbar">
        <button id="toolShutter" class="tool-btn" onclick="toggleShutterPanel()" title="Mode Shutter">📷</button>
        <button id="toolGrid" class="tool-btn" onclick="toggleGridlinePanel()" title="Gridline">▦</button>
        <button id="toolQueue" class="tool-btn" onclick="toggleSidebarMobile()" title="Antrean">📋</button>
        <div class="tool-spacer"></div>
        <button id="toolFullscreen" class="tool-btn" onclick="toggleFullscreen()" title="Fullscreen">⛶</button>
      </div>

      <!-- Target info card (always visible) -->
      ...
      <div class="sidebar-camera-selector">
        ...
        <button class="next-cam-btn" onclick="switchToNextCamera()">▶</button>
        <button class="rescan-btn" onclick="autoDetectBestCamera()">🔄</button>
      </div>

      <!-- Collapsible: Shutter Mode -->
      ...
      <!-- Collapsible: Gridline / Mirror settings -->
      ...
      <!-- Queue list (always visible, fills remaining space, scrollable) -->
      <div class="sidebar-queue queue-panel">
  ...
  <!-- Mobile floating toggle button (shows/hides sidebar on narrow screens) -->
  <button class="queue-toggle-mobile" onclick="toggleSidebarMobile()" title="Antrean">📋</button>
</div>
...
private val OPERATOR_HTML_PART_2 = """
...
private val OPERATOR_HTML_PART_3 = """
...
function updateQueueList() {
  ...
  // Filter by search query (inline search in the sidebar queue)
  if (opSearchQuery) {
...
// ── Sidebar Panel Toggles (replaces old tab switching) ──────────────────
function toggleShutterPanel() {
  ...
  // Close the other panel to save space
  const other = document.getElementById('gridlinePanel')
  if (other && !panel.classList.contains('hidden')) other.classList.add('hidden')
...
// Toggle sidebar visibility on mobile (slide-up bottom sheet)
...
// Listen for fullscreen changes to keep button state in sync
document.addEventListener('fullscreenchange', () => {
  const btn = document.getElementById('toolFullscreen')
  if (btn) btn.classList.toggle('active', !!document.fullscreenElement)
})

// Legacy switchTab/togglePanel — kept as no-ops for backward compatibility
function switchTab(tabName) { /* no-op: split-screen layout has no tabs */ }
function togglePanel(panelId) { /* no-op: split-screen layout has no panels */ }
function initDragDivider() { /* no-op: split-screen layout has no draggable divider */ }

// ── OP Search (filters the queue list inline) ─────────────────────────
let opSearchQuery = ''
...
function renderOpSearchResults() {
  // No-op: search now filters the main queue list inline via updateQueueList().
  // Kept for backward compatibility with callers (e.g. updateQueueList itself).
}
...
val OPERATOR_HTML: String = OPERATOR_HTML_PART_1 + OPERATOR_HTML_PART_2 + OPERATOR_HTML_PART_3
